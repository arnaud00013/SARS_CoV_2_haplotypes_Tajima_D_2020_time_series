...
#Number of cpus for Tajima's D analysis. It corresponds to the number of month that will be analyzed in parallel. Thus, it should be <=7 for Wave 1.
nb_cores <- as.integer(commandArgs(TRUE)[2]) 
nb_cores <- ifelse(test = nb_cores>7,yes=7,no=nb_cores)
...
#focus on first wave
df_metadata_samples <- subset(df_metadata_samples,(as.Date(date)>=as.Date("2020-01-01"))&((as.Date(date)<=as.Date("2020-07-31"))))
...
nb_time_periods <- ceiling(as.numeric(as.Date("2020-07-31")-as.Date("2020-01-01"))/31)
df_time_periods <- data.frame(time_period=1:nb_time_periods,start=as.Date("2020-01-01") %m+% months(time_period_length*(0:(nb_time_periods-1))),stop=(as.Date("2020-01-01")+(time_period_length-1))%m+% months(time_period_length*(1:(nb_time_periods)))-1,Tajima_D=NA,stringsAsFactors = FALSE)
...
cl <- makeCluster(nb_cores,outfile=paste0(output_workspace,"LOG_first_wave_",the_continent,"_top3_haplotypes_resamplings_Tajima_D.txt"))
...
#save table 
write.table(x = df_time_period_Taj_D_with_resamplings,file = paste0(output_workspace,"Table_time_series_Taj_D_with_resamplings_first_wave_",the_continent,"_top3_haplotypes.csv"),row.names = F,col.names = T,sep=",")
...
############################
library("session")
save.session(file = paste0(output_workspace,"Taj_D_first_wave_",the_continent,"_top3_haplotypes_RSession.Rda"))
